...
## Frontend Development Feedback Loop

When doing visual/CSS/layout work on the blog, use the following workflow to see what the site actually looks like and iterate on changes.

### Prerequisites

- Hugo dev server must be running on port 1313 (`hugo server -D --port 1313`)
- Playwright browsers must be installed (`bun run playwright install`)
...
2. **Capture screenshots** to see the result:
   ...
   ```
3. **View screenshots** using the Read tool on files in `tests/screenshots/`
4. **Iterate** — repeat steps 1-3 until the result looks right

### Screenshot inventory

The script captures 12 pages × 2 viewports (desktop Chrome + mobile Safari):
...
### Interactive inspection with playwright-cli

For targeted inspection or interaction testing:
...
- **Templates**: `themes/reallylol/layouts/` (Hugo HTML templates)
...
After making changes, run the full test suite to check for regressions:
